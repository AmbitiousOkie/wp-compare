...
function wpestate_show_extra_options_booking($post_id){
        ...
        if(is_array($extra_pay_options)){
            foreach($extra_pay_options as $key=>$extra_options){
                print'<div class="cost_row cost_row_extra"  data-value_add="'.$extra_options[1].'" data-value_how="'.$extra_options[2].'" data-value_name="'.$extra_options[0].'" >';
                    print '<div class="cost_explanation"> <input type="checkbox" data-key="'.$key.'" class="form-control" value="1" name="checkbox"  /> '.$extra_options[0].' </div>';
                    print '<div class="cost_value"><div class="cost_value_show">'.  wpestate_show_price_booking($extra_options[1] ,$currency,$where_currency,1).'</div> '.wpestate_extra_options_exp($extra_options[2]).' </div>' ;
                print '</div>';
        ...
        if($price_per_guest_from_one == 0 ) {
            ...
            ////////////////////////////////////////////////////////////////
                ...
                if( isset( $price_array[$from_date_unix] ) ) {
                    $has_custom                             =   1;
                    $custom_price_array [$from_date_unix]   =   $price_array[$from_date_unix];
                }
                ...
                $usable_price                           =   wpestate_return_custom_price($from_date_unix,$mega,$price_per_weekeend,$price_array,$price_per_day,$count_days);             
                $total_price                            =   $total_price + $usable_price;
              
                $inter_price                            =   $inter_price + $usable_price;
                $custom_price_array [$from_date_unix]   =   $usable_price;
...
if( !function_exists('second_loop_pagination') ):
...
if( !function_exists('wpestate_check_booking_valability') ):
    function wpestate_check_booking_valability($book_from,$book_to,$listing_id){
        ...
        if($min_days_booking!=0 && $min_days_booking>$days){
            return false;
        }
