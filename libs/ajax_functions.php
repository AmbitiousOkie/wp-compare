...
if( !function_exists('wpestate_send_valid_sms') ):
    function wpestate_send_valid_sms(){
        ...
        if( trim($user_mobile)!='' ){
            ...
            esc_html_e('We sent a sms with a validation code.','wpestate');
        }else{
            esc_html_e('Please add your mobile phone number!','wpestate');
        }
...
if( !function_exists('wpestate_validate_mobile') ):
    function wpestate_validate_mobile(){
        ...
        if($sent_pin==$validate_phoneno){
            esc_html_e('Mobile phone number is validated.','wpestate');
            update_user_meta( $userID, 'check_phone_valid', 'yes') ;
        }else{
            esc_html_e('The entered code is not correct. The mobile phone number is not validated.','wpestate');
        }
   ...
   // set_time_limit(0);
    ...
    if ($menus) {
        foreach ($menus as $menu) {
            if ($menu->name == 'Main Navigation' || $menu->name == 'Main' || $menu->name == 'Primary Menu' || $menu->name == 'Main Menu' || $menu->name == 'main') {
                $menu_locations['primary'] = $menu->term_id;
                $menu_locations['mobile']= $menu->term_id;
            }
            if ($menu->name == 'Footer' || $menu->name == 'footer' || $menu->name == 'Footer Menu') {
...
if( !function_exists('wpestate_ajax_filter_invoices') ):
    function wpestate_ajax_filter_invoices(){
        ...
        $reservation_strings=array(
            ...
            'Listing'                       => esc_html__( 'Listing','wpestate'),
...
if( !function_exists('wpestate_ajax_update_profile') ):
    ...
    function wpestate_ajax_update_profile(){
        ...
        $paypal_payments_to   =   sanitize_text_field ( wp_kses( $_POST['paypal_payments_to'],$allowed_html) );
        $payment_info   =   sanitize_text_field ( wp_kses( $_POST['payment_info'],$allowed_html) );
...
if( !function_exists('wpestate_update_user_agent') ):
    function wpestate_update_user_agent ($agent_id, $firstname ,$secondname ,$useremail,$userphone,$userskype,$profile_image_url,$usermobile,$about_me,$profile_image_url_small,$userfacebook,$usertwitter,$userlinkedin,$userpinterest,$live_in,$i_speak,$login_name,$payment_info) {
...
if( !function_exists('wpestate_ajax_filter_listings') ):
    ...
    function wpestate_ajax_filter_listings(){
        ...
        if (isset($_POST['city']) && trim($_POST['city']) != esc_html__( 'All Cities','wpestate')&& trim($_POST['city']) != 'All Cities' && $_POST['city'] && trim($_POST['city']) != 'all' ) {
        ...
        if ( isset( $_POST['area'] ) && trim($_POST['area']) != esc_html__( 'All Areas','wpestate') && trim($_POST['area']) != 'All Areas' && $_POST['area'] && trim($_POST['area']) != 'all' ) {
