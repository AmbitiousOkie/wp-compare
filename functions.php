...
if (!function_exists('wpestate_modify_contact_methods')):
    ...
    function wpestate_modify_contact_methods($profile_fields) {
        ...
        $profile_fields['check_phone_valid']       = esc_html__('Mobile Verfied','wpestate');
    ...
    WHERE user_activation_key = %s AND user_login = %s", $reset_key, $user_login));
            ...
            if (!empty($user_data)) {
                ...
                if (!empty($reset_key) && !empty($user_data)) {
                    ...
                    $mess = '<div class="login-alert">' . esc_html__( 'A new password was sent via email!', 'wpestate') .$user_mobile. '</div>';
